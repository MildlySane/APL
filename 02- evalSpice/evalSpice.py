...
def file_parse(filename):
    ...
    #List to store Vsource, Isource and Resistors
    V=[]
    I=[]
    R=[]
    ...
    for line in lines:
        if ".end" not in line:
            ...
            if data[0] in V+I+R:
                raise ValueError("Malformed circuit file") #Check the error to be raised;this is if same name happens twice
            ...
            #Add Resistor to list R
                ...
                if(float(data[3])<0):
                    raise ValueError("Negative Resistances not allowed")
